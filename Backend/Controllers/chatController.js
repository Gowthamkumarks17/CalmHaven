...
// ✅ Safe logging (no crash if key missing)
if (config.GEMINI_API_KEY) {
    console.log(
        'Initializing chat controller with API key:',
        config.GEMINI_API_KEY.substring(0, 10) + '...'
    );
} else {
    console.log('⚠️ GEMINI_API_KEY not set - chatbot will not work');
}
...
const relevantKeywords = [
    ...
    'scared', 'afraid', 'worried', 'nervous', 'upset', 'distressed',
    'overwhelmed', 'sad', 'lonely', 'angry', 'hurt', 'confused',
    'hopeless', 'helpless', 'tired', 'exhausted'
];

// Topic check
function isRelevantTopic(message) {
    const msg = message.toLowerCase();
    return relevantKeywords.some(k => msg.includes(k)) ||
...
const chatController = {
    async chat(req, res) {
        try {
            ...
            if (!isRelevantTopic(message)) {
                return res.json({
                    response: "I can help with PTSD and mental health topics. Please ask something related."
                });
            }
...
Respond empathetically, concisely, and only about mental health.
`;
